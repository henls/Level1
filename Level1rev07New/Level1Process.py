...
def Align(DirFits,DirFlat,DirDark,DeviceNumber):
    ...
    try:
        ...
        #开始对齐
        t = 1
        head=fits.getheader(os.path.join(DirFits,data_path_fits[0]))
...
def initial(jsonfile):
    f = open(r'/home/wangxinhua/level1/Level1rev07New/json.txt','r')
    para = json.load(f)
    f.close()
    ...
    global winsr_host, diaratio, start_r0, step_r0, maxfre, filename, sitfdata_host, gussf_host, infrq, otfrq, OnlyBand
    ...
    while 1:
        while 1:
            ...
            try:#判断程序所需文件是否完整且正确
                ...
                if OnlyBand == 0:
                    DirDark = log[12].split('：')[1]
                    if '\n' in DirDark:
                        DirDark = DirDark[:-1]
                    DirFlats = log[13:]#/home/wangxinhua/Desktop/Flat/20200426BFlat.fits等
                else:
                    DirDark = log[8].split('：')[1]
                    if '\n' in DirDark:
                        DirDark = DirDark[:-1]
                    DirFlats = log[9]#/home/wangxinhua/Desktop/Flat/20200426CFlat.fits等
                if OnlyBand == 0:
                    a = len(re.findall(r'[B][F][l][a][t]',DirFlats[0]))
                    b = len(re.findall(r'[C][F][l][a][t]',DirFlats[0]))
                    c = len(re.findall(r'[R][F][l][a][t]',DirFlats[0]))
                    #print(a,b,c)
                    if a != 0:
                        B = DirFlats[0]
                    elif b != 0:
                        ...
                        R = DirFlats[0]
                    a = len(re.findall(r'[B][F][l][a][t]',DirFlats[1]))
                    b = len(re.findall(r'[C][F][l][a][t]',DirFlats[1]))
                    c = len(re.findall(r'[R][F][l][a][t]',DirFlats[1]))
                    #print(a,b,c)
                    if a != 0:
                        B = DirFlats[1]
                    elif b != 0:
                        ...
                        R = DirFlats[1]
                    a = len(re.findall(r'[B][F][l][a][t]',DirFlats[2]))
                    b = len(re.findall(r'[C][F][l][a][t]',DirFlats[2]))
                    c = len(re.findall(r'[R][F][l][a][t]',DirFlats[2]))
                    #print(a,b,c)
                    if a != 0:
                        B = DirFlats[2]
                    elif b != 0:
        ...
        #today_time = datetime.datetime.now().strftime('%Y%m%d')
            ...
            try:
                if OnlyBand == 0:
                    ...
                    worker = myProcessing()
                    while 1:
                        if os.path.exists(os.path.join(Dir_NewFile,today_time+'R.log')) and rr == 0:
                            '''print('满足线程1启动条件')
                            rr = 1
                            thread1 = myThread(R.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'R.log'))
                            thread1.start()
                            print('线程1启动成功')'''
                            rr = 1
                            processing1 = multiprocessing.Process(target=worker.run,args=(R.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'R.log'),0))
                            processing1.start()
                            
                            print('进程1启动成功')
                            #processing1.join()
                        elif os.path.exists(os.path.join(Dir_NewFile,today_time+'C.log')) and cc == 0:
                            '''print('满足线程2启动条件')
                            cc = 1
                            thread0 = myThread(C.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'C.log'))
                            thread0.start()
                            print('线程2启动成功')'''
                            cc = 1
                            processing2 = multiprocessing.Process(target=worker.run,args=(C.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'C.log'),1))
                            processing2.start()
                       
                            print('进程2启动成功')
                            #processing2.join()
                        elif os.path.exists(os.path.join(Dir_NewFile,today_time+'B.log')) and bb == 0:
                            '''print('满足线程3启动条件')
                            bb = 1
                            thread2 = myThread(B.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'B.log'))
                            thread2.start()
                            print('线程3启动成功')'''
                            bb = 1
                            processing3 = multiprocessing.Process(target=worker.run,args=(B.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'B.log'),2))
                            processing3.start()
                            print('进程3启动成功')
                            #processing3.join()
                        elif rr == 1 and bb == 1 and cc == 1:
                            StopFlag = 1
                            break
                else:
                    worker = myProcessing()
                    while 1:
                        if  os.path.exists(os.path.join(Dir_NewFile,today_time+'C.log')):
                            #thread0 = myThread(C.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'C.log'))
                            #thread0.start()
                            processing1 = multiprocessing.Process(target=worker.run,args=(C.replace('\n',''),DirDark,os.path.join(Dir_NewFile,today_time+'C.log'),0))
                            processing1.start()
                            #print('线程1启动成功')
                            print('进程1启动成功')
                            StopFlag = 1

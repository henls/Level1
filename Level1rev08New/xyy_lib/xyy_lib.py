...
def readfits_gpu(filename):
    ...
    header = cp.array(fits.getheader(filename))
    return [data, header]
...
def  cubesrcal_gpu(cubesub,winsr):
    ...
    #cubesp=np.fft.fftshift(np.fft.fft2(cp.asnumpy(cubesubalignwin),axes=(1,2)),axes=(1,2)) 
    #cubesp=cp.array(cubesp)
    ...
    sotf2=np.abs(np.fft.fftshift(np.fft.fft2(cp.asnumpy(lxp))))**2
    sotf2 = cp.array(sotf2)
    sotf2=sotf2/sotf2[srsize//2,srsize//2]
...
def  cubesrdevr0_gpu(cubesr,srsize,winsr,sitfdata,diameter,diaratio,maxfre,low,hig,start_r0,step_r0):
    ...
    r0arr=[]
    ###corsize=[int(srsize*0.9),int(srsize*0.9)]
    cubesr = cp.asnumpy(cubesr)
    winsr = cp.asnumpy(winsr)
    srarry = cp.asnumpy(srarry)
    for i in range(xnum):
        for j in range(ynum):
...
def nvst_dirsandfiles_path(path):
    ...
    if len(target)==3:
        ...
        return [[target[0],target[1]],[path1,path2,path3]]
    elif len(target)==1:
        return [[target[0]],[path1,path2,path3]]
    elif len(target)==0:
        raise MyError('NoFiles')
'''
#---说明：计算功率谱  
#---参数： cubesp 频谱(三维)  
#---返回： psdmean
...
def Psdnd_gpu(psd):
    ...
    #psdnd = np.maximum(cp.asnumpy(psd - 1.0 * noise), 0)
    #psdnd = cp.array(psdnd)
    return psdnd, noise
...
def path_paser(path):
    ...
    for i in roots:
        if 'FLAT' in i.lower() or 'flat' in i.lower():
            ...
            #datapath = fitsfile[t]
        t += 1
    if len(flatpath) == 0 and len(darkpath) != 0 and len(datapath) != 0:
        flatpath = None
        return datapath,flatpath,darkpath
    elif len(darkpath) == 0 and len(flatpath) != 0 and len(datapath) != 0:
        darkpath = None
        return datapath,flatpath,darkpath
    elif len(darkpath) != 0 and len(flatpath) != 0 and len(datapath) == 0:
        datapath = None
        return datapath,flatpath,darkpath
    elif len(flatpath) == 0 and len(darkpath) == 0 and len(datapath) != 0:
        flatpath = None
        darkpath = None
        return datapath,flatpath,darkpath
    elif len(flatpath) != 0 and len(darkpath) == 0 and len(datapath) == 0:
    ...
    while True:
        ...
        if ('dark' in path.lower() or 'Dark' in path.lower()):
            ...
            for i in fitsfilepath:
                data = cp.add(data,cp.array(readfits(path+'/'+i)[0],dtype=cp.float32))
                cal_path.append(i)
            if len(cal_path) == 3000:
                return cp.asnumpy(data/3000)
...
def cubedata(DirFlat,DirDark,pathname,fitsname,rcxsize,rcysize):
    #最新添加log：
    #   数据的预处理data-dark/flat
    DirFlat = DirFlat
    DirDark = DirDark
    Flat = readfits(DirFlat)[0]
    Dark = readfits(DirDark)[0]
    Filter = fitsname
    pathname = pathname
    cube = np.empty([len(Filter),rcxsize,rcysize],dtype=np.float32)
    t = 0
    for i in Filter:
        data = readfits(os.path.join(pathname,i))[0]
        dataProcessed = (data-Dark)/(Flat-Dark)*np.max(Flat-Dark)
        cube[t,:,:] = dataProcessed[0:rcxsize,0:rcysize]
        t += 1
    return cube
...
def JsonWrite(dir_log,key,value):
    ...
    if os.path.exists(dir_logjson) == 0:
